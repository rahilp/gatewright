...
// T-0128 — an issue is a capture like any other, and its author is a
// stranger: `github` is an actor kind `policy.triage_required_for` can name,
// so a board can put a human between what someone opened upstream and what an
// unattended runner starts working on. captureFlag is the one place that
// decides what a bare capture looks like; hardcoding `flag: null` here meant
// `gw add` and `gw sync` disagreed, and a pulled issue was schedulable the
// moment it landed.
function newItem(issue, items, config, stages, warn, now) {
  ...
  return { ...item, flag: captureFlag(item, item.created_by, config) };
}
