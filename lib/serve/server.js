...
import { SETTINGS_BY_KEY, coerce, setValue } from '../settings.js';
import { evaluateCumulative, evaluateRequires, nextStage, stageIndex, stageList } from '../rules.js';
import { describeRule, describeStage } from '../gates/describe.js';
...
function commandRequest(pathname, body, actor) {
  const match = pathname.match(/^\/api\/items\/([^/]+)(?:\/(move|note|triage|resume|edit))?$/);
...
// its own rather than re-derived here: a second evaluator in the viewer is how
// a board starts announcing rules it does not enforce.
function unmetReasons(item, targetStageId, { items, stages }) {
