...
import { SETTINGS_BY_KEY, coerceSetting, setValue, parseGlossaryKey, applyGlossaryEntry } from '../settings.js';
...
// T-0051 — the actor is resolved by the same function every CLI command goes
// through (lib/cli/root.js), not by a serve-private copy: the bare-`agent`
// refusal and the bare-name-as-human:<name> qualification now apply to board
// writes too, and the actor space stays consistent with sameOwner(). The
// import is aliased because the handler below names its local result `actor`.
...
// T-0130 -- A HOST HEADER IS NOT A URL, AND AN ORIGIN IS NOT AN AUTHORITY.
//
// These two headers have different grammars and must be parsed by different
// rules. `Host` is a bare authority ("localhost:7911", "[::1]:7911", "mybox");
// `Origin` is a serialized origin, always carrying a scheme
// ("http://localhost:7911") or the literal "null".
//
// Handing a Host header to `new URL()` does not throw, which is the trap this
// code fell into: "localhost:" parses as a SCHEME, so `new URL('localhost:7911')`
// succeeds with an empty hostname and the allow-list comparison silently failed.
// A bare IP literal is the only authority that does throw -- ":7911" is not a
// legal scheme suffix after digits -- so the split() fallback only ever ran for
// those, and `localhost`, `--host mybox` and `[::1]` all answered 403 on the
// very URL `gw serve` had just printed.
//
// An authority is therefore given a scheme before parsing, and IPv6 brackets are
// stripped so an address compares equal however it arrived.
function hostHeaderName(value) {
  if (typeof value !== 'string') return null;
  const trimmed = value.trim();
  // Only an absolute URL may carry a scheme, a path, a query, a fragment or
  // userinfo. A Host header containing any of them is malformed, and guessing at
  // what it meant is how an allow-list gets talked around: "attacker@localhost"
  // must never be read as "localhost".
  if (!trimmed || /[/?#@\\\s]|:\/\//.test(trimmed)) return null;
  return parse(`http://${trimmed}`);
}

// An Origin has to be an absolute URL. The literal "null" (an opaque origin) and
// a bare authority are both refused here rather than coerced into a hostname:
// every real browser sends a scheme, so anything else is not a browser and has
// no CSRF story to protect.
...
// browser will later send bracketed. Bracket it before parsing so both spellings
// land on the same canonical hostname.
function allowEntry(value) {
  if (typeof value !== 'string') return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  const bare = trimmed.includes(':') && !trimmed.startsWith('[') && !/^[^:]+:\d+$/.test(trimmed);
  return hostHeaderName(bare ? `[${trimmed}]` : trimmed);
}
...
function hostPermitted(allowed, host) {
  const hostname = hostHeaderName(host);
  return hostname != null && allowed.has(hostname);
}

function originPermitted(allowed, origin) {
  const hostname = originName(origin);
  return hostname != null && allowed.has(hostname);
}
...
//
// `hostPermitted(allowed, ...)` is deliberately widened by --host: that is how a
...
function adminAllowed(req) {
  return hostPermitted(LOOPBACK, req.headers.host)
    && (!req.headers.origin || originPermitted(LOOPBACK, req.headers.origin));
}
...
function writeAllowed(req, allowed) {
  ...
  return hostPermitted(allowed, req.headers.host) && Boolean(req.headers.origin) && originPermitted(allowed, req.headers.origin) && contentType === 'application/json';
}

function requestActor(body, env) {
  // One resolution, not a serve-private one: `by`/`actor` from the request
  // body stand in for `--by`, then the same rules the CLI applies.
  return resolveActor({ by: body.by ?? body.actor }, env);
}
...
function setPaused(store, paused, actor) {
  return store.withLock(() => {
    ...
    // writeConfig, not a direct write: it re-baselines the digest, so a
    // pause from the board is never reported as an out-of-band write.
...
function replaceStages(store, body, actor) {
  ...
  return store.withLock(() => {
    ...
    // A legitimate stages write must not be reported as tampering by the next
    // `gw check`, so the digest is re-baselined with the new pipeline.
...
function setSettings(store, body, actor) {
  ...
  return store.withLock(() => {
    ...
    for (const key of keys) {
      ...
      // coerceSetting, not coerce: for list settings the bracketed JSON form
      // `gw config` prints is accepted and validated here too (T-0050) --
      // comma-splitting it would store quoted garbage that then rejects the
      // very values the setting lists as allowed.
      const result = coerceSetting(setting, Array.isArray(raw) ? raw.join(',') : raw);
      if (result.error) throw new UsageError(result.error);
    ...
    const released = persistConfigAndReleaseTriageHolds(store, { previousConfig, config, actor });
    store.appendEvent({ type: 'config', by: actor, keys });
    return { ok: true, config: applied, released };
  });
}

function errorResponse(res, error) {
  // T-0080: a browser panel prints error.message verbatim, so a RuleError may
  // carry a browserMessage that states the same refusal as a decision the
  // reader can act on (T-0018) -- the CLI wording, commands and all, stays in
  // the terminal where commands are runnable advice.
  if (error instanceof RuleError) return sendJson(res, 409, { error: error.browserMessage ?? error.message, failures: error.failures });
...
function createSyncController({ store, env, stderr = process.stderr, ghRun, clock = {}, syncFn } = {}) {
  ...
  // "off" and "configured but never scheduled" are different facts and must not
  // report the same way. A board whose config says github.enabled: true while
  // the header says nothing at all is how an issue sits unsynced for days with
  // no indication anything is wrong.
  const unscheduled = Boolean(github.enabled && github.repo && !intervalMs);
  const state = {
    enabled, unscheduled, intervalMin: intervalMs ? intervalMin : null, status: enabled ? 'idle' : unscheduled ? 'unscheduled' : 'off',
...
export function createServeServer({ store, shell = readFileSync(new URL('../../viewer/board.html', import.meta.url), 'utf8'), env = process.env, cwd = store.root, stderr = process.stderr, ghRun, clock, syncFn, allowedHosts = [] } = {}) {
  ...
  // accepted what that means. Default construction is unchanged. Entries are
  // normalised through the same parser the headers go through, so an IPv6
  // address given as `--host ::1` still matches the bracketed `[::1]:<port>` a
  // browser puts in Host.
  const allowed = new Set([...LOOPBACK, ...allowedHosts.map(allowEntry).filter(Boolean)]);
  ...
  const server = createServer(async (req, res) => {
    ...
    if (!hostPermitted(allowed, req.headers.host) || (req.headers.origin && !originPermitted(allowed, req.headers.origin))) {
    ...
    // A FIRST LOAD IS NOT A POLL, AND IT USED TO CARRY THE WHOLE LOG.
    //
    // `?since` has always capped the incremental polls, but a full page reload
    // sends no cursor -- the board starts from the pinned shell with no state --
    // so a year-old board answered every reload with its entire history:
    // 13MB+ at the audited 2,000-item/100k-event scale, JSON.stringify'd
    // synchronously on the same single thread the scheduler ticks on.
    //
    // The cap is the split `gw open` and `gw gc --events` already make, taken
    // from the same helper rather than reimplemented: every event of an open
    // item, the tail of each finished one. Sharing it is the point -- a
    // snapshot, a compacted board and the live board must not disagree about
    // which history is hot. It also means nothing an open item needs is ever
    // capped away, so the Play/Cancel control still reads its own dispatches.
    //
    // `?since` is untouched. Capping an incremental poll would drop events the
    // viewer appends rather than replaces, and it would never see them again.
    const { kept, archived } = since
      ? { kept: events.filter((event) => String(event.ts) > since), archived: [] }
      : partitionEvents(events, { open: openItemIds(items, stages), keep: eventsKeep(config) });
    // Same field names and same home as the snapshot path (lib/viewer/inject.js
    // puts them in the config block), so the viewer has one place to read them
    // whichever way the board was loaded. Written only when something really was
    // left out: an uncompacted board's response is unchanged.
    const omission = archived.length ? { eventsOmitted: archived.length, eventsArchive: basename(store.paths.eventsArchive) } : {};
    return send(res, 200, JSON.stringify({
      // Ownership is evaluated on the server with the same rule release uses.
      // The viewer needs this one capability bit to avoid offering Release to
      // someone the server will refuse; it does not get to reimplement owner
      // matching in browser JavaScript.
      items: items.map((item) => ({ ...item, can_release: Boolean(item.owner && sameOwner(item.owner, resolveActor({}, env))) })),
      events: kept,
      stages: withGateDescriptions(stages), config: { ...config, ...omission }, sync: sync.status(), generatedAt: new Date().toISOString(),
