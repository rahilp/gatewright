...
export const VOCAB_FIELDS = ['type', 'phase', 'priority'];
  ...
  for (const field of VOCAB_FIELDS) {
    ...
    for (const [value, count] of [...counts].sort((a, b) => b[1] - a[1])) {
      findings.push({
        ...
        fix: `${count === 1 ? '1 item has' : `${count} items have`} ${field} ${JSON.stringify(value)}, which is not in vocab.${field} (${allowed.join(', ')}). Either widen it with \`gw config vocab.${field} "${[...allowed, value].join(',')}"\` or correct the items with \`gw edit <id> --${field} <value>\`.`,
      });
    }
  }
  return findings;
