...
import { evaluateCumulative, nextStage, stageIndex, stageList, stageOrderMessage, TRIAGE_FLAGS } from '../rules.js';
...
export function run(ctx) {
  ...
  const result = ctx.store.withLock(() => {
    ...
    // "still unreviewed".
    // The same holds for an unclassified capture (T-0113): once finished,
    // there is nothing left for the scheduler to hold it back from.
    if (TRIAGE_FLAGS.includes(moved.flag) && isTerminalStage(to, stages, roles)) moved.flag = null;
    items[index] = moved;
