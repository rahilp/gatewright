...
export function run(ctx) {
  ...
  const result = ctx.store.withLock(() => {
    ...
    if (current.flag === 'needs-triage' && !ctx.flags.force && !sideTarget && to !== roles.initial) {
      ...
      const error = new RuleError(`item ${id} is held for triage (flagged needs-triage by ${current.created_by ?? 'an unknown actor'}): someone other than its creator must run \`gw triage ${id} --approve\` before it advances past ${roles.initial}. Pass --force to move it anyway as a deliberate override.`);
      error.browserMessage = `item ${id} is held for triage (flagged needs-triage by ${current.created_by ?? 'an unknown actor'}): someone other than its creator has to approve the hold before it advances past ${roles.initial}.`;
      throw error;
    }
