...
export function run(ctx) {
  ...
  const result = ctx.store.withLock(() => {
    ...
    if ((stages.terminal ?? []).includes(current.stage) && (!ctx.flags.force || !sideTarget)) throw new RuleError(`item ${id} is finished in terminal stage ${current.stage}; if this is a mistake, run \`gw move ${id} <side-stage> --force\``);
