...
import { evaluateCumulative, findCycles, missingDeps, stageList, FLAGS, TRIAGE_FLAGS } from '../rules.js';
...
export function run(ctx) {
  ...
  // does fail.
  //
  // T-0113 — the inbox holds both triage flags. An `unclassified` capture can
  // be worked, a `needs-triage` hold cannot; the heading counts them apart.
  // A needs-triage flag that an older gw put on a human's unclassified
  // capture is reported on its own, because its remedy is `gw repair`.
  const legacy = new Set(legacyCaptureHolds(items, config, (item) => isTerminalStage(item.stage, stages, roles)).map((item) => item.id));
  for (const item of items) {
    if (!TRIAGE_FLAGS.includes(item.flag)) continue;
    if (isTerminalStage(item.stage, stages, roles)) {
      notes.push(problem('stale triage hold', item.id, `finished in ${item.stage} with the ${item.flag} flag, which no longer needs action: run \`gw repair --write\` to clear it`));
      continue;
    }
    if (legacy.has(item.id)) {
      notes.push(problem('old capture hold', item.id, `captured by ${item.created_by} without a classification, and an older gw held it for review as if an agent had created it: run \`gw repair --write\` to release it`));
      continue;
    }
    const fix = triageAdvice(item, ctx.actor);
    const rotted = Date.parse(item.updated) < cutoff;
    const age = item.flag === 'unclassified' ? 'not classified' : 'untriaged';
    (rotted ? problems : notes).push({ ...problem(rotted ? 'needs triage' : 'inbox', item.id, rotted ? `${age} for over ${staleDays} days: ${fix}` : fix), flag: item.flag });
  }
  ...
  if (!ctx.flags.json && notes.length) {
    ...
    for (const [type, entries] of groups) {
      ...
      if (type === 'inbox') {
        const unclassified = entries.filter((entry) => entry.flag === 'unclassified').length;
        const held = count - unclassified;
        const kinds = [
          ...(unclassified ? [`${unclassified === count ? plural(count, 'item') : unclassified} not classified yet`] : []),
          ...(held ? [`${held === count ? plural(count, 'item') : held} held for review`] : []),
        ];
        const summary = kinds.length > 1 ? `${plural(count, 'item')} waiting for triage (${kinds.join(', ')})` : kinds[0];
        ctx.stdout.write(`INBOX — ${summary}. Not a violation; nothing to do unless you want to.\n`);
      } else if (type === 'stale triage hold') {
        ctx.stdout.write(`STALE TRIAGE HOLD — ${plural(count, 'finished item')} still ${count === 1 ? 'carries' : 'carry'} a triage flag. Not a violation; run \`gw repair --write\` to tidy ${count === 1 ? 'it' : 'them'} up.\n`);
      } else if (type === 'old capture hold') {
        ctx.stdout.write(`OLD CAPTURE HOLD — ${plural(count, 'unclassified item')} ${count === 1 ? 'is' : 'are'} still held the way gw before 0.13 held every unclassified capture. Not a violation; run \`gw repair --write\` to release ${count === 1 ? 'it' : 'them'}.\n`);
      } else {
        ctx.stdout.write(`QUEUED DISPATCH — ${plural(count, 'dispatch', 'dispatches')} no agent will ever run while the runner is disabled.\n`);
      }
