...
export function run(ctx) {
  ...
  const active = items.filter((item) => !terminal.has(item.stage));
  ...
  for (const entry of missingDeps(active)) problems.push(problem('missing dependency', entry.id, `add or remove missing dependency: ${entry.missing.join(', ')}`));
  for (const cycle of findCycles(active)) problems.push(problem('dependency cycle', cycle[0], `break dependency cycle: ${cycle.join(' -> ')}`));
  for (const item of active) {
