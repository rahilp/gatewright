...
export function run(ctx) {
  ...
  if (!ctx.flags.json && notes.length) {
    ctx.stdout.write(`INBOX — ${notes.length} item${notes.length === 1 ? '' : 's'} not classified yet. Not a violation; nothing to do unless you want to.\n`);
    for (const entry of notes) ctx.stdout.write(`  ${entry.id}: ${entry.fix}\n`);
  }
