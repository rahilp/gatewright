...
export function run(ctx) {
  ...
  for (const item of items) {
    ...
    // T-0039 — the advice used to name `gw edit` and `gw claim`, neither of
    // which clears the flag: needs-triage is a hold on unreviewed work, and
    // both of those are commands an agent runs too, so letting them lift the
    // hold would let the reviewed party review itself. Only triage does, and
    // only triage must be named -- the advice has to survive being followed.
    const fix = `run \`gw triage ${item.id} --approve\` to clear the hold, or \`gw triage ${item.id} --drop\` to discard it`;
  ...
  if (!ctx.flags.json && notes.length) {
    ...
    for (const [type, entries] of groups) {
      ...
      if (type === 'inbox') {
        ...
        ctx.stdout.write('  To clear a hold, run `gw triage <id> --approve`; to discard it, run `gw triage <id> --drop`.\n');
      } else {
      ...
      for (const entry of shown) {
        if (type === 'inbox') ctx.stdout.write(`  ${entry.id}\n`);
