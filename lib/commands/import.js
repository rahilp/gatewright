...
export function resolveImportStage(item, intendedStageId, { items, stages }) {
  ...
  if (intendedStageId === 'backlog') return { stage: intendedStageId, reason: null };
  ...
  return { stage: 'backlog', reason: targetVerdict.failures[0] ?? cumulativeVerdict.failures[0] };
}

export function resolveImportStages(items, stages) {
  const byId = new Map(items.map((item) => [item.id, { ...item, intendedStage: item.stage }]));
...
function printDowngrades(downgrades, stdout) {
  for (const d of downgrades) {
    const sourceDescription = d.source === 'verified' ? 'source says done' : `source says ${d.source}`;
    stdout.write(`${d.id}: ${sourceDescription}, imported to ${d.stage} (${d.source} ${humanReason(d.reason)})\n`);
  }
  const verifiedDowngrades = downgrades.filter((d) => d.source === 'verified');
  if (verifiedDowngrades.length) {
    stdout.write(`${verifiedDowngrades.length} item(s) marked done in the source could not enter verified; see above.\n`);
  }
}
...
export async function run(ctx) {
  ...
  if (flags['dry-run']) {
    ...
    printDowngrades(downgrades, stdout);
