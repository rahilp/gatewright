...
export async function run(ctx) {
  ...
  assignIds(vocabKept, config, store.readItems().map((i) => i.id));
  const knownIds = new Set([...store.readItems().map((i) => i.id), ...vocabKept.map((i) => i.id)]);
  const { items, skipped: depSkipped } = enforceDeps(vocabKept, knownIds);
  const skipped = [...parsed.skipped, ...vocabSkipped, ...depSkipped];
