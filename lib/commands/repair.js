...
import { stageList, FLAGS, TRIAGE_FLAGS, isUnclassified } from '../rules.js';
import { legacyCaptureHolds } from '../triage-policy.js';
...
// Flags gw itself left behind, which repair tidies without being asked twice:
// a triage flag on finished work (it can no longer mean anything), and the
// needs-triage hold gw before 0.13 put on every unclassified capture by a
// non-agent (T-0113), which now becomes the `unclassified` flag such a
// capture earns today -- or no flag, if the item was classified since.
function flagTidyups(store) {
  try {
    const stages = readStages(store);
    const roles = resolveRoles(stages);
    const config = readConfig(store);
    const items = store.readItems();
    const finished = (item) => isTerminalStage(item.stage, stages, roles);
    return {
      stale: items.filter((item) => TRIAGE_FLAGS.includes(item.flag) && finished(item)),
      legacy: legacyCaptureHolds(items, config, finished),
    };
  } catch {
...
function applyFlagTidyups(store, actor) {
  const { stale, legacy } = flagTidyups(store);
  if (!stale.length && !legacy.length) return { stale, legacy };
  const now = new Date().toISOString();
  const flags = new Map([
    ...stale.map((item) => [item.id, { flag: null, reason: 'stale triage hold cleared from finished item' }]),
    ...legacy.map((item) => [item.id, { flag: isUnclassified(item) ? 'unclassified' : null, reason: 'pre-0.13 capture hold released; unclassified capture is not held' }]),
  ]);
  store.writeItems(store.readItems().map((item) => flags.has(item.id) ? { ...item, flag: flags.get(item.id).flag, updated: now } : item));
  for (const [id, { flag, reason }] of flags) {
    store.appendEvent({ type: 'flag', item: id, flag, by: actor, repaired_by: actor, reason });
  }
  return { stale, legacy };
}

function describeTidyups({ stale, legacy }, stdout) {
  for (const item of stale) stdout.write(`items.jsonl: ${item.id} is finished in ${item.stage} but still has ${item.flag}\n`);
  for (const item of legacy) stdout.write(`items.jsonl: ${item.id} was captured by ${item.created_by} without a classification and is still held with needs-triage by gw before 0.13\n`);
}

function reportTidyups({ stale, legacy }, stdout) {
  if (stale.length) stdout.write(`Repaired: cleared ${plural(stale.length, 'stale triage hold')} from finished work; recorded flag events.\n`);
  if (legacy.length) stdout.write(`Repaired: released ${plural(legacy.length, 'pre-0.13 capture hold')}, so ${legacy.length === 1 ? 'that item' : 'those items'} can be worked now; recorded flag events.\n`);
}
...
export function run(ctx) {
  ...
  const tidyups = flagTidyups(store);
  if (!findings.length) {
    if (tidyups.stale.length || tidyups.legacy.length) {
      if (!ctx.flags.write) {
        describeTidyups(tidyups, stdout);
        const found = [
          ...(tidyups.stale.length ? [`${plural(tidyups.stale.length, 'stale triage hold')} on finished work`] : []),
          ...(tidyups.legacy.length ? [`${plural(tidyups.legacy.length, 'pre-0.13 capture hold')}`] : []),
        ];
        stdout.write(`Found ${found.join(' and ')}. Dry run — nothing changed. Run again with --write to clear ${tidyups.stale.length + tidyups.legacy.length === 1 ? 'it' : 'them'} and record flag events.\n`);
        return 0;
      }
      return store.withLock(() => {
        const applied = applyFlagTidyups(store, ctx.actor);
        if (!applied.stale.length && !applied.legacy.length) {
          stdout.write('Board files are clean. Nothing to repair.\n');
          return 0;
        }
        reportTidyups(applied, stdout);
        return 0;
      });
    }
  ...
  if (!ctx.flags.write) {
    report(findings);
    stdout.write(`Found ${plural(findings.length, 'bad line')} (${corrupt.length} unparseable, ${invalid.length} valid JSON but invalid content). Dry run — nothing changed.\nRun again with --write to quarantine the unparseable ${corrupt.length === 1 ? 'one' : 'ones'} in .gatewright/quarantine.jsonl and repair the board.\n`);
  ...
  return store.withLock(() => {
    ...
    if (quarantined.length && digestAfter.status === 'clean') stdout.write(`Repaired: ${plural(quarantined.length, 'bad line')} moved to .gatewright/quarantine.jsonl; nothing was deleted; prior digest restored.\n`);
    if (quarantined.length && rebaselined) stdout.write(`Repaired: ${plural(quarantined.length, 'bad line')} moved to .gatewright/quarantine.jsonl; nothing was deleted; digest re-baselined by explicit recovery.\n`);
    if (quarantined.length && digestAfter.status === 'modified' && !ctx.flags.force) {
      stdout.write(`Repaired: ${plural(quarantined.length, 'bad line')} moved to .gatewright/quarantine.jsonl; nothing was deleted; digest remains stale because surviving board content differs from the prior digest. Run \`gw check\` to inspect it. After deliberate review, run \`gw repair --write --force\` to re-baseline.\n`);
    }
    ...
    reportTidyups(applyFlagTidyups(store, ctx.actor), stdout);
