...
import { TRIAGE_FLAGS } from '../rules.js';
...
export function run(ctx) {
  ...
  return ctx.store.withLock(() => {
    ...
    // T-0113 — approving an unclassified capture takes it as it is: the flag
    // clears and the scheduler may pick it up. Its creator may do that too;
    // isOwnAgentTriage never matches it, because agents do not earn the flag.
    if (!TRIAGE_FLAGS.includes(item.flag)) throw new RuleError(`item ${id} is not held for triage`);
