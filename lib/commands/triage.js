...
export function run(ctx) {
  ...
  return ctx.store.withLock(() => {
    ...
    if (approve && defaultActorInCursor(ctx)) {
      throw new RuleError(`refusing to approve ${id}: Cursor Agent is present but GW_ACTOR is unset, so this approval would be recorded as ${ctx.actor}. Set \`GW_ACTOR=agent:<name>\` and retry as a different agent, ask a human to approve it, or turn off holds for agent-created items with \`gw config policy.triage_required_for '[]'\`. Identity is declared, not authenticated.`);
    }
