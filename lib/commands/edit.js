...
import { findCycles, isUnclassified } from '../rules.js';
...
export function run(ctx) {
  return ctx.store.withLock(() => {
    ...
    // T-0113 — the unclassified flag says exactly "no phase, type or
    // priority yet", so an edit that supplies one of them retires it. The
    // needs-triage policy hold is untouched: only `gw triage` lifts that.
    const classified = next.flag === 'unclassified' && !isUnclassified(next);
    if (classified) next.flag = null;
    ...
    ctx.store.appendEvent({
      ...
      by: ctx.actor,
      fields: changed,
      ...(classified ? { flag: null, reason: 'classified' } : {}),
