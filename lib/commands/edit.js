...
const githubOwnedFields = ['title', 'scope', 'priority', 'type', 'phase', 'gate'];
...
export function run(ctx) {
  return ctx.store.withLock(() => {
    ...
    for (const field of vocabValidatedFields) {
      const allowed = config.vocab?.[field];
      if (
        ctx.flags[field] !== undefined
        && Array.isArray(allowed)
        && !allowed.includes(ctx.flags[field])
      ) {
        throw new UsageError(
          `invalid --${field} '${ctx.flags[field]}'; allowed values: ${allowed.join(', ')}`,
        );
      }
    }
