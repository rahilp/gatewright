...
import { SETTING_HELP } from '../settings-help.js';
...
const SECTIONS = { runner: 'AI agents', policy: 'Limits on agent work', guard: 'Commit check', id_scheme: 'Item numbers', brief: 'Brief', check: 'Health check', memory: 'Memory', github: 'GitHub', vocab: 'Word lists' };
  ...
  return {
    ...
    view(state, theme) {
      ...
      return {
        body,
        focusLine,
        description: SETTING_HELP[setting.key]?.effect ?? setting.summary,
        warning: SETTING_HELP[setting.key]?.danger ?? setting.danger,
...
// The shipped defaults, for the "Default:" line in each editor. Read from the
// template init writes, so the editor can never quote a default that a fresh
// board does not actually have.
let shippedDefaults;
// Keys the solo and team pipelines set differently at init have no single
// default, so none is claimed for them.
const PIPELINE_SET = new Set(['policy.triage_required_for', 'policy.auto_dispatch_children']);
function defaultFor(key) {
  if (PIPELINE_SET.has(key)) return undefined;
  shippedDefaults ??= JSON.parse(readTemplate('config.json'));
  return getValue(shippedDefaults, key);
}
...
// What a value of an enum-like setting does. Declared values come from the
// setting itself; provider choices are described by what they would run.
function choiceDetail(setting, config, choice) {
  const declared = SETTING_HELP[setting.key]?.values?.[choice];
  if (declared) return declared;
  const providers = setting.key === 'runner.provider' ? config.runner?.providers : setting.key === 'memory.provider' ? config.memory?.providers : null;
  const entry = providers?.[choice];
  if (entry?.cmd) return `Runs this command: ${[entry.cmd].flat().join(' ')}`;
  if (entry?.url) return `Connects to ${entry.url}${entry.token_env ? `, using the key in $${entry.token_env}` : ''}`;
  return null;
}

// One setting's editor, as its own screen. The question is the key; under it,
// what the setting changes in practice; under each value, what choosing that
// value does. Esc here means "back to the list" rather than "abandon
// everything": the list is where a user decides that.
async function editOne(prompter, setting, config) {
  const help = SETTING_HELP[setting.key] ?? {};
  const current = getValue(config, setting.key);
  const fallbackValue = defaultFor(setting.key);
  const choices = choicesFor(setting, config);
  const meta = { step: sectionFor(setting.key), warning: help.danger ?? setting.danger, explain: help.effect ?? setting.summary };
  const defaultNote = fallbackValue === undefined ? '' : ` Default: ${shown(fallbackValue)}.`;
  let next;
  if (setting.type === 'boolean') {
    const mark = (value) => (value === fallbackValue ? ' (default)' : '');
    next = await prompter.select(setting.key, [
      { value: true, label: `On${mark(true)}`, detail: help.values?.true },
      { value: false, label: `Off${mark(false)}`, detail: help.values?.false },
    ], { ...meta, fallback: current === true ? 0 : 1 });
  } else if (choices?.length) {
    next = await prompter.select(setting.key, choices.map((choice) => ({
      label: choice === fallbackValue ? `${choice} (default)` : choice,
      value: choice,
      detail: choiceDetail(setting, config, choice),
    })), { ...meta, fallback: Math.max(0, choices.indexOf(current)) });
  } else if (setting.type === 'list') {
    const empty = setting.allowEmpty ? ' Type none for an empty list.' : ' At least one value.';
    const answer = await prompter.text(`${setting.key} (comma-separated)`, {
      ...meta,
      description: `${help.values?.entry ?? ''}${empty}${defaultNote}`.trim(),
      fallback: Array.isArray(current) ? current.join(', ') : '',
      validate: (raw) => (raw === '' && !setting.allowEmpty ? 'enter at least one value' : coerceSetting(setting, raw).error),
    });
    next = coerceSetting(setting, answer).value;
  } else {
    const range = setting.type === 'integer' ? `Allowed: ${setting.min}–${setting.max}${help.unit ? ` ${help.unit}` : ''}.` : '';
    const answer = await prompter.text(setting.key, {
      ...meta,
      description: `${range}${defaultNote}`.trim() || undefined,
      fallback: current === undefined || current === null ? '' : String(current),
      validate: (raw) => (raw === '' ? 'enter a value' : coerce(setting, raw).error),
    });
    next = coerce(setting, answer).value;
  }
...
async function edit(ctx, config, previousConfig) {
  const prompter = createPrompter({ input: ctx.stdin, output: ctx.stdout, env: ctx.env ?? process.env, platform: ctx.platform, release: ctx.osRelease, title: 'gw config' });
