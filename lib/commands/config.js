...
import { SETTINGS, SETTINGS_BY_KEY, coerce, getValue, setValue, parseGlossaryKey } from '../settings.js';
...
function glossaryEntry(ctx, config, target, value) {
  ...
  if (description) {
    ...
    // description that says nothing is worse than no description, because it
    // takes up the space where the explanation should be.
    delete config.glossary[field][code];
  }
  persist(store, config);
  stdout.write(description ? `${key} = ${display(description)}\n` : `${key} removed\n`);
  const vocab = config.vocab?.[field];
  if (description && Array.isArray(vocab) && !vocab.includes(code)) {
    stdout.write(`Note: ${code} is not in vocab.${field}, so nothing on the board will show this yet.\n`);
  }
  return 0;
}
