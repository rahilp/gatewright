...
export function run(ctx) {
  ...
  return store.withLock(() => {
    ...
    const phase = flags.phase ?? (parent?.phase) ?? config.vocab?.phase?.[0] ?? null;
    const id = nextId(items, { scheme: config.id_scheme ?? 'phase-seq', phase, parent: flags.parent });
    const now = new Date().toISOString();
    const item = { id, title, phase, priority: flags.priority ?? null, gate: flags.gate ?? null, type: flags.type ?? null, stage: roles.initial, flag: null, owner: null, scope: flags.scope ?? '', deps: [], evidence: [], notes: '', refs: [], parent: flags.parent ?? null, created_by: actor.startsWith('agent:') ? actor : 'human', gh: null, created: now, updated: now };
