...
function vocabCheck(config, flags) {
  for (const field of ['type', 'phase', 'priority', 'gate']) {
    const allowed = config.vocab?.[field];
    if (flags[field] !== undefined && Array.isArray(allowed) && !allowed.includes(flags[field])) throw new UsageError(`invalid --${field} '${flags[field]}'; allowed values: ${allowed.join(', ')}`);
  }
}
