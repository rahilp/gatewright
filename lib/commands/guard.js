...
function report(ctx, verdict, { subject, mode }) {
  ...
  // T-0132 — a refusal may own its headline. "This change is not on the
  // board" is wrong for a commit that names a finished item: that item is on
  // the board, it is just done, and a sentence the author can see is untrue
  // is how a gate loses its authority.
  ctx.stderr.write(`${lead} ${label}${verdict.headline ?? 'this change is not on the board.'}\n`);
...
function pretool(ctx, { git, readStdin, config, stages, items, branch }) {
  ...
  return deny(ctx, [
    ...
    '  2. gw add "<the change>"  (one item per step you plan to take; --parent <id> for sub-steps)',
...
export function run(ctx, { git = createGit(), readStdin = () => readFileSync(0, 'utf8') } = {}) {
  ...
  if (ctx.flags.range) {
    subjects = commitsIn(git, cwd, ctx.flags.range).map((commit) => ({
      ...
      actor: null,
      // specs §6.5: a range reads history rather than gating an action, so an
      // item that has since been finished still accounts for its own
      // commits — a pull request is reviewed after its work is done. This is
      // the one place a finished id is not a refusal (T-0132).
      history: true,
    }));
  } else {
    const message = ctx.flags['message-file']
      ...
      : (ctx.flags.message ?? '');
    subjects = [{ label: '', sha: null, message, files: stagedFiles(git, cwd), actor: ctx.actor, history: false }];
  }

  const results = subjects.map((subject) => ({
    ...subject,
    verdict: guardCommit({ message: subject.message, branch, files: subject.files, items, actor: subject.actor, stages, config, history: subject.history }),
  }));
