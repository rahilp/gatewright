...
function pretool(ctx, { git, readStdin, config, stages, items, branch }) {
  ...
  const within = path && isAbsolute(path) ? relative(ctx.root, path) : path;
  // An edit to the board itself, or to anything outside the repository, is not
  // the kind of work a board tracks.
  if (!path || within.startsWith('..')) return 0;
...
export function run(ctx, { git = createGit(), readStdin = () => readFileSync(0, 'utf8') } = {}) {
  ...
  const results = subjects.map((subject) => ({
    ...subject,
    verdict: guardCommit({ message: subject.message, branch, files: subject.files, items, actor: subject.actor, stages, config, retrospective: Boolean(ctx.flags.range) }),
  }));
  ...
  if (ctx.flags.json) {
    ...
    for (const result of refused) report(ctx, result.verdict, { subject: result.label, mode });
  }
