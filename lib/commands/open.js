...
export const spec = {
  ...
  flags: { 'no-browser': { type: 'boolean' }, watch: { type: 'boolean' }, port: { type: 'string' }, 'all-events': { type: 'boolean' } },
  positionals: [],
};
...
// T-0135 — the snapshot inlines the event log, so the log's growth is the
// document's growth: at the audited 2,000-item / 100k-event scale that is a
// 13.2MB board.html, nearly all of it the history of finished items. The
// split is the one `gw gc --events` makes (every event of an open item, the
// tail of each finished one), so a compacted board and a fresh snapshot agree
// on what history is hot. `--all-events` is the escape hatch for anyone who
// wants the whole log in one file.
function snapshot(store, { allEvents = false } = {}) {
  ...
  const { kept, archived } = allEvents
    ? { kept: events, archived: [] }
    : partitionEvents(events, { open: openItemIds(items, stages), keep: eventsKeep(config) });
  const html = injectData(shell, {
    items,
    events: kept,
    stages: withGateDescriptions(stages),
    config,
    generatedAt: new Date().toISOString(),
    eventsOmitted: archived.length,
    eventsArchive: basename(store.paths.eventsArchive),
  });
  return { html, items };
}
...
function rebuild(store, stdout, { quiet = false, previousItems = [], allEvents = false } = {}) {
  const { html, items } = snapshot(store, { allEvents });
...
function watchBoard(store, stdout, previousItems, { allEvents = false } = {}) {
  let timer = null;
  let closed = false;
  ...
  const rebuildSoon = () => {
    ...
    timer = setTimeout(() => {
      timer = null;
      try {
        previousItems = rebuild(store, stdout, { quiet: true, previousItems, allEvents });
      } catch (err) {
...
export async function run(ctx) {
  ...
  const allEvents = Boolean(flags['all-events']);
  const items = rebuild(store, stdout, { allEvents });
  stdout.write(`${store.paths.board}\n`);
  if (flags.watch) watchBoard(store, stdout, items, { allEvents });
