...
const SEPARATOR = '·';
// The plain markdown checklist — `- [ ] task` / `- [x] task` — is the most
// common task format there is, and most of those files have no `## P<n> —`
// headings at all. They are accepted anywhere: under a phase heading the item
// inherits that phase; with no headings the item gets none and import mints
// its id (T-0047).
...
export function parseMarkdown(text) {
  ...
  let phase = null;
  // Deliberate exclusion, not absence: under a Parking lot heading nothing is
  // imported, checklist lines included. A plain checklist file has no
  // headings at all, which is not the same thing.
  let parkingLot = false;
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (checkbox && !parkingLot) {
      items.push({
        // The physical file line, kept only for import's skip report; never
        // written to the board (fullItems maps explicit fields).
        line: i + 1,
        // No id in a checklist: import mints one from the board's id scheme.
        id: null,
        title: checkbox[2].trim(),
        phase,
        priority: null,
        type: null,
        // A ticked box claims completion; the stage resolver still demands
        // the evidence and downgrades what it cannot honour, exactly like a
        // ✅ in the six-field format.
        stage: checkbox[1] === ' ' ? 'initial' : 'done',
        scope: '',
        deps: [],
      });
      continue;
