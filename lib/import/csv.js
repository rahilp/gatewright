...
const ALIASES = new Map([
  ...
  // T-0134 — promised by PRD R8 and P1-12, absent from this map until now,
  // so a spreadsheet's notes column was read and then thrown away. The
  // aliases are the column names an export actually uses; `description` is
  // deliberately NOT one of them (it already means scope above, and one
  // header cannot mean two fields).
  ['notes', 'notes'], ['note', 'notes'], ['comments', 'notes'],
  ['refs', 'refs'], ['ref', 'refs'], ['references', 'refs'],
  ['parent', 'parent'], ['parent_id', 'parent'], ['parent_item', 'parent'],
]);
...
export function parseCsv(text) {
  ...
  for (let r = 1; r < rows.length; r += 1) {
    ...
    items.push({
      ...
      // See lib/import/json.js: evidence is what earns a stage, so dropping
      // it downgrades finished work and blames the user for it. A CSV cell is
      // plain text, so entries arrive as the migrated legacy shape —
      // `{ text, stage: null }`, exactly what lib/store.js reads a
      // pre-T-0029 board into — and lib/rules.js decides what such an entry
      // may satisfy. Normalised here rather than written back as bare
      // strings so an imported item lands on disk in the same shape a
      // `gw move` writes.
      evidence: record.evidence
        ? record.evidence.split(/[,;]/).map((entry) => entry.trim()).filter(Boolean).map((text) => ({ text, stage: null }))
        : [],
      // A notes cell is free text — RFC 4180 quoting is exactly what lets it
      // hold the newlines an item's running notes are made of — so it is
      // carried verbatim rather than split on anything.
      notes: record.notes || '',
      refs: record.refs ? record.refs.split(/[,;]/).map((entry) => entry.trim()).filter(Boolean) : [],
