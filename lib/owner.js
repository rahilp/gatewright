...
export function triageAdvice(item, actor) {
  const id = item.id;
  // T-0113 — an unclassified capture is not a hold on the work: it keeps the
  // item off the scheduler, and classifying it or approving it as is clears
  // that. Anyone may do either, its creator included.
  if (item.flag === 'unclassified') {
    return `it can be worked now; set its phase, type or priority with gw edit, or run \`gw triage ${id} --approve\` to take it as is`;
  }
  if (isOwnAgentTriage(item, actor)) {
    return `you cannot approve your own item; run \`gw triage ${id} --drop\` to discard it, ask a human or a different agent to approve it, or turn off holds for agent-created items with \`gw config policy.triage_required_for none\``;
  }
