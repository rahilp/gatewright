...
export function evaluateRequires(item, targetStageId, { items = [], stages, supplied = null, actor = null, dependencyAdvice = true } = {}) {
  ...
  if (requires.evidence_min && fresh.length < requires.evidence_min) {
    ...
    //
    // T-0129 — and they are placeholders, never a string that would pass. The
    // literal `--evidence "new evidence 1"` printed here cleared this gate as
    // printed, which made the evidence rule satisfiable by pasting back the
    // refusal. The placeholder names the shape the stage's own pattern gate
    // wants, so on a stage that carries both rules one filled-in command
    // clears both rather than clearing one and being refused by the other.
    const still = requires.evidence_min - fresh.length;
    const flags = Array.from({ length: still }, (_, index) =>
      `--evidence ${evidencePlaceholder(requires.evidence_match, { index, of: still })}`).join(' ');
    failures.push(
