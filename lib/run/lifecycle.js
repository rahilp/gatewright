...
function looksLikeRecordedRun(run, { platform, windowsStartTimeFn, pidReuseToleranceMs } = {}) {
  if (!alive(run.pid)) return false;
  if (platform === 'win32') {
    const liveStart = windowsStartTimeFn(run.pid);
    if (liveStart == null) return false;
    const recordedStart = Date.parse(run.started);
    return !Number.isFinite(recordedStart) || Math.abs(liveStart - recordedStart) <= pidReuseToleranceMs;
  }
  if (platform !== 'linux' || !run.worktree) return true;
...
function signal(run, name, identityOptions) {
  if (!looksLikeRecordedRun(run, identityOptions)) return false;
...
export function createRunLifecycle({
  store, registry = createRunRegistry({ store }), gitHead = defaultGitHead, gitMessage = defaultGitMessage, wait = pause, memoryTransport,
  ...
  function checkIdentity(run) {
    return looksLikeRecordedRun(run, identityOptions);
  }
  ...
  function requestStop(run) {
    if (platform === 'win32') taskkillFn(run.pid, { force: false });
    else signal(run, 'SIGTERM', identityOptions);
  }
  function forceStop(run) {
    if (platform === 'win32') taskkillFn(run.pid, { force: true });
    else signal(run, 'SIGKILL', identityOptions);
  }
  ...
  function end(run, { outcome }) {
    ...
    const claimed = registry.take(run);
    if (!claimed || !checkIdentity(claimed)) {
    ...
    if (checkIdentity(claimed)) forceStop(claimed);
