...
function looksLikeRecordedRun(run, { platform, windowsStartTimeFn, pidReuseToleranceMs, commandTimeoutMs } = {}) {
  ...
  if (platform === 'win32') {
    ...
    if (probe.timedOut) return true;
    if (probe.startTime == null) return false;
    const recordedStart = Date.parse(run.started);
    return !Number.isFinite(recordedStart) || Math.abs(probe.startTime - recordedStart) <= pidReuseToleranceMs;
  }
...
export function createRunLifecycle({
  ...
  function end(run, { outcome }) {
    ...
    if (!claimed || !checkIdentity(claimed, commandTimeoutMs)) {
      return { status: 'already_stopped', run };
    }
    requestStop(claimed, commandTimeoutMs);
    wait(configuredTimeout(config) * 1000);
    ...
    if (checkIdentity(claimed, commandTimeoutMs)) forced = forceStop(claimed, commandTimeoutMs);
