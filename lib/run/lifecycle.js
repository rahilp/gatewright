...
function recorded(registry, run) {
  return registry.list().records.find((candidate) => candidate.run === run.run && candidate.pid === run.pid && candidate.item === run.item) ?? null;
}
...
export function createRunLifecycle({ store, registry = createRunRegistry({ store }), gitHead = defaultGitHead, wait = pause } = {}) {
  ...
  function end(run, { outcome }) {
    if (!recorded(registry, run) || !looksLikeRecordedRun(run)) {
      registry.clear(run.run);
    ...
    const latest = recorded(registry, run);
    if (latest && looksLikeRecordedRun(latest)) signal(latest, 'SIGKILL');

    store.withLock(() => {
      const items = store.readItems(); const item = items.find((candidate) => candidate.id === run.item);
      if (item) {
        item.prev_stage = item.stage;
        const paused = resolveRoles(readStages(store)).paused;
        if (paused) item.stage = paused;
        item.flag = 'paused'; item.owner = null; item.last_commit = gitHead(run.worktree); item.updated = new Date().toISOString();
        store.writeItems(items);
      }
      store.appendEvent({ type: 'run_ended', item: run.item, run: run.run, outcome, by: `agent:${run.run}` });
      registry.clear(run.run);
    });
  ...
  return { stopItem, stopAll, enforceTimeouts, resume, logTail };
}
