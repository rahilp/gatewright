...
// A run's commit is read from the checkout, not from a git process, because
// this is reached from the kill switch. That only works if the read models
// where git actually keeps things:
//
//   * A linked worktree's `.git` is a FILE pointing at a private git dir
//     (`<repo>/.git/worktrees/<id>`) which holds only per-worktree state --
//     HEAD, index, its own logs. Branch refs, packed-refs and every object
//     live in the repository's COMMON dir, which the `commondir` file beside
//     HEAD names. T-0131: the first version resolved `refs/heads/<branch>`
//     and `objects/` inside the private dir, where they never are, so every
//     run on every worktree recorded `last_commit: null` and every memory
//     note said "changed:" with nothing after it.
//   * A plain checkout's `.git` is a DIRECTORY, and reading a directory as a
//     file throws EISDIR -- which the old catch turned into the same silent
//     null. Both shapes are handled here.
//
// Returns { gitDir, commonDir }; they are the same directory outside a
// linked worktree.
function gitDirs(worktree) {
  ...
  try {
    if (statSync(dotGit).isDirectory()) gitDir = dotGit;
      ...
      gitDir = resolve(worktree, pointer.slice('gitdir:'.length).trim());
    }
  } catch { return null; }
  let commonDir = gitDir;
  // `commondir` holds a path relative to the private dir ("../..") in every
  // worktree git creates, and an absolute one is equally legal.
...
// Loose file first, then packed-refs, looking in both directories: a
// worktree owns a few refs of its own, every branch belongs to the common
// dir, and a repository that has been packed has no loose file at all.
function readRef(dirs, ref) {
  if (!ref || ref.includes('..')) return null;
  for (const dir of [dirs.gitDir, dirs.commonDir]) {
    const value = readTrimmed(join(dir, ref));
    if (value) return value;
  }
  for (const dir of [dirs.commonDir, dirs.gitDir]) {
...
// Detached HEAD holds the sha itself; otherwise it is a symbolic ref, and a
// symbolic ref may (rarely) point at another one. Bounded rather than
// recursive so a cyclic ref cannot spin the kill switch.
function headSha(dirs) {
  let value = readTrimmed(join(dirs.gitDir, 'HEAD'));
  for (let hops = 0; value && hops < 5; hops += 1) {
    if (!value.startsWith('ref: ')) return /^[0-9a-f]{40,64}$/i.test(value) ? value : null;
    value = readRef(dirs, value.slice(5).trim());
  }
  return null;
}

// The fallback, and only the fallback. Reading the files keeps completion
// offline and out of a subprocess, which is what a kill switch wants, but it
// cannot answer for a layout these reads do not model -- most of all a
// commit whose object has been packed, where the message lives inside a
// .pack and reading it would mean reimplementing git's index format. Asking
// git costs a process on that rare path and nothing on the common one.
//
// It is asked through lib/git.js and nowhere else. Process spawning in this
// project stays auditable in one place per kind -- run/spawn.js owns the
// agent processes (and test/run-spawn.test.js pins that nothing else under
// lib/run imports child_process), git.js owns the git ones, argv-array and
// already bounded by its own timeout. A second, private execFileSync here
// would be a third place to audit and an unbounded hang waiting to be
// reintroduced.
function gitOutput(git, cwd, argv) {
  try {
    return git.run(argv, { cwd }).stdout.trim() || null;
  } catch { return null; }
}

function defaultGitHead(worktree, { git = createGit() } = {}) {
  if (!worktree) return null;
  const dirs = gitDirs(worktree);
  return (dirs && headSha(dirs)) ?? gitOutput(git, worktree, ['rev-parse', 'HEAD']);
}
...
function defaultGitMessage(worktree, { git = createGit() } = {}) {
  if (!worktree) return '';
  const dirs = gitDirs(worktree);
  const sha = dirs && headSha(dirs);
  if (sha) {
    // Objects live in the common dir -- a linked worktree has no `objects/`
    // of its own at all, which is why the private-dir read this replaces
    // returned '' for every worktree run.
    try {
      const raw = inflateSync(readFileSync(join(dirs.commonDir, 'objects', sha.slice(0, 2), sha.slice(2)))).toString('utf8');
      const subject = raw.slice(raw.indexOf('\0') + 1).split(/\r?\n\r?\n/)[1]?.split(/\r?\n/)[0]?.trim();
      if (subject) return subject;
    } catch {}
  }
  return gitOutput(git, worktree, ['log', '-1', '--format=%s']) ?? '';
}
...
export function createRunLifecycle({
  ...
  function resume(id) {
    return store.withLock(() => {
      ...
      // Resume restores exactly what completeStop produces: flag 'paused',
      // prev_stage recorded, owner already cleared. Anything else was never
      // paused, so there is nothing to restore — and unconditionally clearing
      // the owner here would silently release another actor's live claim.
      if (item.flag !== 'paused' || !item.prev_stage) {
        throw new Error(`item ${id} is not paused; resume only restores an item that \`gw stop\` paused. Run \`gw show ${id}\` to inspect it.`);
      }
      item.stage = item.prev_stage;
      item.flag = null; item.updated = new Date().toISOString();
      store.writeItems(items);
