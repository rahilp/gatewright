...
// realpath where the path exists, the lexical path otherwise — an absent path
// cannot be resolved and is not equal to any registered worktree anyway.
...
export function createWorktree({ git = createGit() } = {}) {
  function ensure({ root, item, config = {} }) {
    ...
    // git reports worktree paths with symlinks resolved. On macOS a repository
    // under /var or /tmp is really /private/var or /private/tmp, so comparing
    // an unresolved path against git's output never matches and we would
    // conclude a registered worktree is a stray directory.
    const existing = listed.find((entry) => realPath(entry.path) === realPath(path));
