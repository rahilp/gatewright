...
// The tick's hot path, exported so it can be measured and exercised on its own.
// tick() is the only production caller and calls exactly this -- the cost that
// matters here is CHOOSING a candidate over a whole board, and timing a whole
// tick() instead measures admission's items.jsonl rewrite (atomic and fsync'd
// since T-0136, so it scales with item count and with the host's durability
// cost) rather than this.
export function selectCandidate({ config, stages, items, events, active = new Set() }) {
  const context = { config, stages, items, events, outstanding: outstandingIndex(events) };
  return items.filter((item) => !active.has(item.id) && eligible(item, context)).sort((a, b) => (
...
export function createScheduler({ store, runner = createRunner(), registry = createRunRegistry({ store }), lifecycle = createRunLifecycle({ store, registry }), worktree = createWorktree(), makeRunId = defaultRunId, stdout = process.stdout } = {}) {
  ...
  function tick() {
    ...
    const active = new Set(initialRecords.map((record) => record.item));
    const candidate = selectCandidate({ config, stages, items, events, active });
