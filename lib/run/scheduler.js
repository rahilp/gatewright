...
export function createScheduler({ store, runner = createRunner(), registry = createRunRegistry({ store }), worktree = createWorktree(), makeRunId = defaultRunId, stdout = process.stdout } = {}) {
  ...
  function tick() {
    ...
    const run = makeRunId();
    const started = runner.start({ config, item: candidate, run, worktree: prepared.path, root: store.root, registry, stages, items, promptValues: { target_stage: nextStage(stages, candidate.stage) } });
    store.withLock(() => {
