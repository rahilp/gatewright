...
export function createScheduler({ store, runner = createRunner(), registry = createRunRegistry({ store }), lifecycle = createRunLifecycle({ store, registry }), worktree = createWorktree(), makeRunId = defaultRunId, stdout = process.stdout } = {}) {
  ...
  function tick() {
    ...
    const run = makeRunId();
    const started = runner.start({ config, item: candidate, run, worktree: prepared.path, root: store.root, registry, onExit: lifecycle.finish, stages, items, promptValues: { target_stage: nextStage(stages, candidate.stage), ...dispatchPromptValues(events, candidate.id) } });
    store.withLock(() => {
