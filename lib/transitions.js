...
//
// The target stage is judged as a move with no evidence supplied (T-0029):
// everything recorded on the item is prior, so a stage the item re-enters --
// say, after a forced move backward -- shows the fresh-evidence refusal a
// real `gw move` would give. Earlier gates in the scope are judged on the
// entries that earned them, exactly as cumulative evaluation does.
export function unmetReasons(item, targetStageId, { items, stages }) {
  ...
  for (const stage of scope) {
    const supplied = stage.id === targetStageId ? item.evidence ?? [] : null;
    for (const [key, value] of Object.entries(stage.requires ?? {})) {
      if (evaluateRequires(item, stage.id, { items, stages: isolateRule(stages, stage.id, key, value), supplied }).ok) continue;
...
export function transitionsFor(item, items, stages) {
  ...
  for (const target of stageList(stages)) {
    ...
    // Judged as a move that supplies no evidence: recorded entries are prior
    // for the target stage, matching what a real `gw move` without --evidence
    // would answer (T-0029).
    const verdict = evaluateCumulative(item, target.id, { items, stages, supplied: item.evidence ?? [] });
