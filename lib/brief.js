...
export function inFlightTitles({ items = [], stages = {} }) {
  return items.filter((item) => !terminal(item, stages) && (item.owner || stageIndex(stages, item.stage) > 0)).map((item) => item.title).filter(Boolean);
}
