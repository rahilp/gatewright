...
export function computeBuckets(state, options = {}) {
  ...
  const inFlight = open.filter((item) => !dispatched.includes(item) && !blocked.includes(item) && item.flag !== 'needs-triage' && isInFlight(item, stages, active) && (!options.me || sameOwner(item.owner, options.me)));
  ...
  return { open, dispatched, blocked, triage, inFlight, next };
}
...
function blockedSuffix(item, items, stages) {
  ...
  return dep ? `waiting on ${dep} (${stageName(stages, items.find((candidate) => candidate.id === dep)?.stage)})` : 'flagged blocked';
}
...
export function renderBrief(state, options = {}) {
  ...
  const { open, dispatched, blocked, triage, inFlight, next } = computeBuckets(state, options);
  ...
  const sections = [
    ...
    [`NEEDS TRIAGE (${triage.length})`, triage, (item) => `created by ${item.created_by ?? 'an unknown actor'} — run \`gw triage ${item.id} --approve\` to clear the hold, or \`gw triage ${item.id} --drop\` to discard it`],
  ...
  if (!emitted) {
    ...
    lines.push('', open.length
      ...
      : (finished
        ? `All ${finished} item(s) are finished. Start new work with \`gw add "title"\`.`
...
export function briefJson(state, options = {}) {
  ...
  const { dispatched, blocked, triage, inFlight, next } = computeBuckets(state, options);
  const byId = new Map(items.map((item) => [item.id, item]));
  const payload = {
    open: items.filter((item) => !terminal(item, stages)).length,
    ...
    blocked: blocked.map((item) => {
      ...
      return { id: item.id, title: sanitizeTitle(item.title), flag: item.flag ?? null, waiting_on: waitingOn, waiting_on_stage: waitingOn ? byId.get(waitingOn)?.stage ?? null : null };
    }),
