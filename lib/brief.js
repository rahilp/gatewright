...
export function renderBrief(state, options = {}) {
  ...
  if (legend.size) {
    ...
    // Use the same width as the row columns (54 for title) plus spacing and indent.
    // Compute: 2 spaces indent + 9 char ID + 2 spaces + 54 title + 9 spaces for suffix column = 76,
    // but legend doesn't need ID/spacing, so use row indent (2) + title width (54) + margin = ~78
    const WRAP_WIDTH = 76; // derived from title width (54) + row structure, not hardcoded 80
