...
export function computeBuckets(state, options = {}) {
  ...
  const next = rawOpen.filter((item) => !dispatched.includes(item) && !blocked.includes(item) && !triage.includes(item) && !item.owner && stageIndex(stages, item.stage) <= 0 && depsReady(item, items, stages) && isMine(item));
...
export function renderBrief(state, options = {}) {
  ...
  const lines = [`gw · ${open.length} open · ${inFlight.length + dispatched.length} in flight · ${blocked.length} blocked${finishedPart}${gitPart}`];
  const sections = [
  ...
  let remaining = Math.max(0, maxLines - (2 + footer.length)); // header, blank line, plus the footer
