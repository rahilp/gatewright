import { nextStage, stageIndex, TRIAGE_FLAGS } from './rules.js';
...
export function computeBuckets(state, options = {}) {
  ...
  // T-0113 — the inbox holds both triage flags: a policy hold and a capture
  // nobody has classified yet. Only the first stops the work; the row says which.
  const triage = rawOpen.filter((item) => TRIAGE_FLAGS.includes(item.flag) && isMine(item));
...
export function renderBrief(state, options = {}) {
  ...
  const sections = [
    ...
    [`NEEDS TRIAGE (${triage.length})`, triage, (item) => `${item.flag === 'unclassified' ? 'not classified yet, ' : ''}created by ${item.created_by ?? 'an unknown actor'} — ${triageAdvice(item, options.actor)}`],
...
export function briefJson(state, options = {}) {
  ...
  const payload = {
    ...
    needs_triage: triage.map((item) => ({ id: item.id, title: sanitizeTitle(item.title), flag: item.flag, created_by: item.created_by ?? null })),
