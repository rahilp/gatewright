...
function stageName(stages, id) { return stages?.stages?.find((stage) => stage.id === id)?.label ?? id ?? 'unknown'; }
function terminal(item, stages) { return (stages?.terminal ?? []).includes(item.stage) || item.stage === 'dropped'; }
