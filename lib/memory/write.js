...
export function memoryRecord({ root, config = {}, item, from, to, commitMessage }) {
  ...
  for (const value of item.evidence ?? []) {
    // Entries are `{ text, stage }` on disk (T-0029); flat strings are the
    // migrated shape. The memory contract only ever sees the text.
    const safe = safeEvidence(typeof value === 'string' ? value : value?.text);
