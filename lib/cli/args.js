  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (!positionalOnly && arg.startsWith('--') && arg.length > 2) {
      ...
      if (definition.type === 'boolean') {
        ...
        // T-0069 — a flag that declares optionalValue may stand alone: `gw
        // brief --me` means "the actor I resolved as", so a missing value
        // (end of argv, the next flag, or an explicit '') yields `true` for
        // the command to interpret, not a parse error.
        if (definition.optionalValue === true && (value === undefined || value === '' || (equals === -1 && value.startsWith('--')))) {
          value = true;
        } else if (value === undefined || (value === '' && !emptyAllowed) || (equals === -1 && value.startsWith('--'))) throw new UsageError(`flag --${name} needs a value`);
      }
