...
usage: gw <command> [options]
  ...
  show <id> [--json]                    read one item and its events
  list [--stage S] [--owner W] [--limit N] [<text>] read many: filtered, searched, and capped
  check [--json]                        report rule violations and out-of-band writes
  doctor [--json] [--port P]            check gw, the board, and the hooks, and name what fixes each
  ...
  gc [--events] [--dry-run]             archive finished items' old events; without --events, remove terminal-stage worktrees
  ...
  mcp [--by agent:<name>]               serve the board to an MCP client over stdio
  ...
  deprecation warning. When GW_ROOT picks a board other than the one found
  from the current directory, gw names it on stderr.
