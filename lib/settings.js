...
export const SETTINGS = [
  ...
  { key: 'policy.triage_required_for', type: 'list', allowEmpty: true, summary: 'Actor kinds whose created items are held for triage. Use none (or []) for no policy holds.' },
  ...
  { key: 'github.mirror_children', type: 'boolean', summary: 'Open a GitHub issue for each agent-created child of a linked item.' },
...
export function coerceList(setting, raw) {
  ...
  const syntax = `expected a JSON array of strings like ["a","b"], or a comma-separated list like a,b${setting.allowEmpty ? '; use none or [] for an empty list' : ''}`;
  if (setting.allowEmpty && text.toLowerCase() === 'none') return { value: [] };
  // cmd.exe passes single quotes through literally. Without this guard, a
  // pasted POSIX spelling such as '[]' becomes the one-item list ["'[]'"]
  // through the comma-separated path instead of failing visibly.
  if (/^(['"])\[\]\1$/.test(text)) return { error: `${setting.key}: quoted [] is not valid. ${syntax}` };
